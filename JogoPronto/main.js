...
document.addEventListener('keydown', function(event) {
  if (event.keyCode === 87) { // W
    if (tabuleiro[y-1][x] !== '*' && tabuleiro[y-1][x] !== "D" && !chaveEncontrada || tabuleiro[y-1][x] !== '*' && chaveEncontrada ) {
      y--;
      verificarChave();
    }
    ...
    if (tabuleiro[y][x-1] !== '*' && tabuleiro[y][x-1] !== 'D' && !chaveEncontrada || tabuleiro[y][x-1] !== '*' && chaveEncontrada) {
      x--;
      verificarChave();
    }
  ...
  } else if (event.keyCode === 49) { // Tecla 1
    interagir = true;
    verificarInteracao();
  }
});
...
function verificarChave() {
  ...
  if (tabuleiro[y][x] === '#') {
    ...
    if (vidas == 0) {
      alert("GAME OVER")
      window.location.href = "indexDerrota.html"
    }
  }
  ...
  for (var i = 0; i < tabuleiro.length; i++) {
    for (var j = 0; j < tabuleiro[i].length; j++) {
      ...
      if (tabuleiro[i][j] === '*') {
        ...
        ctx.fillText('Casseb', (bossx - 0.3) * tamQuadrados, (bossy + 0.1) * tamQuadrados);
        ...
        ctx.fillStyle = '#fff';
        ctx.fillRect(j * tamQuadrados, i * tamQuadrados, tamQuadrados, tamQuadrados);
      }
