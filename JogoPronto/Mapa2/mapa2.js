...
function redirecionarParaOutroMapa() {
  window.location.href = "/JogoPronto/Mapa3/mapa3.html";
}

// FUNÇÃO DO BOTÃO
function verificarInteracao() {
  if (tabuleiro[y][x] === "O" && interagir) {
    alert("Foi ser curioso e liberou o GIROTTO!");
    bossAtivo = true;
    bossx = 24;
    bossy = 21;
...
function verificarBoss() {
  if (y === bossy && x === bossx && bossAtivo == true) {
    --vidas;
    alert(`Você foi pego pelo Girotto! Agora você só tem ${vidas} Restantes`);
    reiniciarJogo();
    return;
  }
}
